...
import com.droidhubworld.crashlogger.activity.LogMessageActivity;
...
public class CrashLogAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.crash_log_item, null);
    ...
    private class CrashLogViewHolder extends RecyclerView.ViewHolder {
        ...
        void setUpViewHolder(final Context context, final File file) {
            ...
            String log = FileUtils.readFirstLineFromFile(new File(filePath));
            try {
                JSONObject data = new JSONObject(log);
                textViewMsg.setText(data.optString(Constants.EXCEPTION_SUFFIX));

            } catch (JSONException e) {
                e.printStackTrace();
                textViewMsg.setText(log);
            }
